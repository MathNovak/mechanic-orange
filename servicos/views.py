...
def gerar_os(request, identificador):
    ...
    pdf.cell(190, 10, 'PitStop Express', 1, 1, 'C', 1)

    pdf.cell(37, 10, 'Cliente:', 1, 0, 'L', 1)
    pdf.cell(0, 10, f'{servico.cliente.nome}', 1, 1, 'L', 1)

    pdf.cell(37, 10, 'Manutençoes:', 1, 0, 'L', 1)

    categorias_manutencao = servico.cateagoria_manutencao.all()
    ...
    for i, manutencao in enumerate(categorias_manutencao):
        ...
        if not i == len(categorias_manutencao) -1:
            pdf.cell(37, 10, '', 0, 0) # 8 ep 17:34 

    pdf.cell(37, 10, 'Data de início:', 1, 0, 'L', 1)
    pdf.cell(0, 10, f'{servico.data_inicio}', 1, 1, 'L', 1)

    pdf.cell(37, 10, 'Data de entrega:', 1, 0, 'L', 1)
    pdf.cell(0, 10, f'{servico.data_entrega}', 1, 1, 'L', 1)

    pdf.cell(37, 10, 'Protocolo:', 1, 0, 'L', 1)
    pdf.cell(0, 10, f'{servico.protocolo}', 1, 1, 'L', 1)    

    pdf.cell(37, 10, 'Valor total:', 1, 0, 'L', 1)
    ...
    pdf_bytes = BytesIO(pdf_content)
          ...
          # para download direto: as_attachment=True
